...
Until the target year is reached, net dividends are fully reinvested. Baked into the portfolio growth formula at every simulation step.

---

### 7. Portfolio Breakdown
Visual breakdown of how your final portfolio is composed:
- **Contributions** — capital you put in yourself
- **DRIP Returns** — dividends reinvested over time
- **Price Growth** — market appreciation

Each component shows its dollar value and percentage share with an animated bar.

---

### 8. Export CSV
Download the full year-by-year projection table as a `.csv` file — includes all columns (and Real/Mo if inflation is enabled). Filename auto-matches your selected year range.

---

### 9. Share
Generate a URL with all your current parameters encoded. Anyone who opens the link sees your exact simulation — no backend required.

---

## How the Simulation Works

The core simulation runs year by year:

```
1. avgPort = portValue + (monthly × 12 × 0.5)
   — approximates contributions arriving spread throughout the year
...
4. portValue = portValue × (1 + priceGrowth) + (monthly × 12) + divNet
   — new portfolio value: price appreciation + contributions + DRIP

5. divYield = min(divYield × (1 + divCAGR), 0.15)
   — yield grows with dividend CAGR, capped at 15%

6. netMonthly = divNet ÷ 12
   — monthly take-home

7. realMonthly = netMonthly ÷ (1 + inflation)^year
   — real purchasing power (shown when inflation toggle is on)
```
...
- Dividend yield compounds at a constant CAGR
- Price appreciation is constant year-over-year
- Tax rate is flat
...
- `Space Grotesk` — numbers and data
...
5. Use **Export CSV** to download the full projection table
6. Use **Share** to copy a link with your exact parameters
...
© 2026 · [github.com/GavrielP1](https://github.com/GavrielP1)
